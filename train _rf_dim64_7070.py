from dit import DiT
from unet_cat import UnetConcat
...
from model_net import RectifiedFlow, LogitNormalCosineScheduler
...
def main():
    ...
    experiment = Experiment(
        ...
        project_name="xxx"  # 🚀 新实验：15k样本验证修复版
    )
    ...
    model = UnetConcat(
        ...
        cond_drop_prob=0.3,  # CFG 条件丢弃概率（与 training_cfg_rate 一致）
        ...
        for step in pbar:
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                ...
                if contrastive_weight > 0 and b > 1:
                    ...
                    u_positive_flat = u_positive.flatten(start_dim=1)
                    u_negative_flat = u_negative.flatten(start_dim=1)
                    ...
                    # 计算对比损失
                    pos_sim = (pred_norm * u_pos_norm).sum(dim=1)
                    neg_sim = (pred_norm * u_neg_norm).sum(dim=1)
                    ...
                    base_loss = base_loss + contrastive_weight * contrastive_loss
