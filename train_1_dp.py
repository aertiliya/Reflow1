...
def main():
    ...
    experiment = Experiment(
        ...
        project_name="xlx"  # 🚀 新实验：15k样本验证修复版
    )
    ...
    model = UnetConcat(
        channels=1,
        dim=128,  # 从160提升至192（增强表达能力，解决欠拟合）
        dim_mults=(1, 2, 4, 8),
    ...
    progressive_regularizer = ProgressiveRegularizer(
        ssim_threshold_low=0.99,  # ⬆️ 从0.85提到0.90（只有达到0.9才启动dropout）
        ssim_threshold_high=0.999,  # ⬆️ 从0.90提到0.95（目标更高）
        gap_threshold_high=0.15,  # ⬆️ 放宽gap容忍度到0.15
        gap_threshold_low=0.05,
        dropout_min=0.0,
        dropout_max=0.0,  # ⬇️ 从0.25降到0.15（大幅降低正则化强度）
        dropout_step=0.0  # ⬇️ 从0.05降到0.03（更缓慢的增长）
    )

    # 🚀 初始化自适应损失调度器（Requirements 5.1-5.5）
    # 🔥 优化：60k步后SSIM权重继续增长，突破平台期
    adaptive_loss_scheduler = AdaptiveLossScheduler(
        phase1_end=30000,  # 阶段1：纯L1（0-30k步）
        phase2_end=80000,  # ⬆️ 从60k延长到80k（给SSIM更长的增长期）
        ssim_weight_min=0.3,
        ssim_weight_max=0.85,  # ⬆️ 从0.7提到0.85（更强的SSIM约束）
        ssim_weight_boost=0.9,  # 停滞时提升至0.9
        ssim_weight_reduce=1.0,  # 下降时降低至0.5
    ...
    lr_controller = LearningRateController(
        ...
        warmup_steps=20000,  # Warmup 10k步（比原来5k更平滑）
        cosine_period=120000,  # ⬆️ 从80k延长到100k（学习率衰减更慢）
        min_lr_ratio=0.3,  # ⬆️ 从0.2提到0.3（最低学习率更高）
        restart_ratio=0.5,  # 停滞时重启至初始值50%
        decay_ratio=1.0  # 下降时衰减至当前值50%
    )
    ...
    def sample_and_log_images(step, save_images=False):
        ...
        with torch.no_grad():
            ...
            # 并行采样：一次性处理所有训练样本
            train_samples = sampler_dm.sample_each_class(2, cfg_scale=1.0, seis=train_seis)
            ...
            for i in range(2):
                ...
                # 从并行采样结果中提取对应样本
                pred = train_samples[i].squeeze(0).squeeze(0)
            ...
            # 并行采样：一次性处理所有测试样本
            test_samples = sampler_dm.sample_each_class(2, cfg_scale=1.0, seis=seis_batch)
            ...
            for i, sample_id in enumerate(sample_ids):
                ...
                # 从并行采样结果中提取对应样本
                pred = test_samples[i].squeeze(0).squeeze(0)
                gt = vel_gt.squeeze(0).squeeze(0).squeeze(0)

                # 存储预测和真值
                test_preds_all.append(test_samples[i])
                test_gts_all.append(vel_gt)
            ...
            # ========================================
            # 3. 保存图像（当step % 5000 == 0时触发）
            # 使用已采样的预测结果，避免重复采样
            # ========================================
            if step % 5000 == 0:
                # 使用已采样的测试集预测结果
                vis_preds = torch.cat(test_preds_all, dim=0)
                vis_preds = vis_preds.unsqueeze(1)
                vis_gts = torch.cat(test_gts_all[:2], dim=0)  # 取前2个样本 (2, 1, 64, 64)

                # 保存预测图像
                log_img_pred = make_grid(vis_preds, nrow=2, normalize=True, value_range=(-1, 1))
                img_save_path_pred = "/images_openfwi/step_test_pred.png"
                ...
                img_save_path_gt = "/images_openfwi/step_test_gt.png"
    ...
    def forward_diffusion(x0, timesteps, num_diffusion_timesteps=1000, device="cpu"):
        # 1. 生成线性调度的 betas（与 SimpleDiffusion 一致）
        ...
        # 2. 计算 alpha 相关参数
        alphas = 1 - betas
        alpha_cumulative = torch.cumprod(alphas, dim=0)  # α_bar_t = Π(α_s) for s=1 to t
        ...
        # 获取对应时间步的参数（对应原代码中的 get() 函数）
        sqrt_alpha_cumulative_t = alpha_cumulative[timesteps].sqrt()
        sqrt_one_minus_alpha_cumulative_t = (1 - alpha_cumulative[timesteps]).sqrt()

        # 4. 调整维度以便与图像进行广播运算
        # 原始图像 x0 维度: [batch_size, C, H, W]
        # 参数需要调整为: [batch_size, 1, 1, 1] 或 [len(timesteps), 1, 1, 1]
        while len(sqrt_alpha_cumulative_t.shape) < len(x0.shape):
            sqrt_alpha_cumulative_t = sqrt_alpha_cumulative_t.unsqueeze(-1)
        ...
        noisy_images = sqrt_alpha_cumulative_t * x0 + sqrt_one_minus_alpha_cumulative_t * noise
    ...
    # 🚀 梯度平衡器：EMA 平滑的 Loss 比值
    base_loss_ema = None
    edge_loss_ema = None
    ema_momentum = 0.99  # EMA 动量

    # 🚀 灰图检测：记录SSIM历史，连续1000步SSIM < 0.1时发出警告
    ...
    # 🚀 控制变量实验：前 50k 步学习率曲线与旧实验完全对齐
    max_steps_for_validation = 100000

    # 🔥 关键修复：训练循环从 start_step 开始（而不是 0）
    with tqdm(range(start_step, n_steps), dynamic_ncols=True, initial=start_step, total=n_steps) as pbar:
        ...
        for step in pbar:
            ...
            ts = torch.randint(low=1, high=1000, size=(x1.shape[0],), device=device)
            xts, gt_noise = forward_diffusion(x1, ts, device=device)

            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                pred = model.forward(x=xts, t=ts, y=y, seis=seis)
                positive_loss = F.mse_loss(pred, gt_noise)
                base_loss = positive_loss
                loss_ssim = positive_loss
