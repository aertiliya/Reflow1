...
if __name__ == "__main__":
    # 1. 加载npy文件
    npy_file_path = r"C:\pycharm_project\Reflow2\seis6_1_35_0.npy"  # 替换为实际的npy文件路径
    np_array = np.load(npy_file_path)
    total_elements = np_array.size
    np_array_reshaped = np_array.reshape(1, 5, 1000, 70).astype(np.float32)
    seis = torch.from_numpy(np_array_reshaped)
    seis = torch.clamp(seis, -1, 1)

    unet = InversionNet()
    unet.eval()

    out = unet.forward(x=seis)

    # 降维：去掉batch维度(第0维)和通道维度(第1维)，得到70x70的2维数组
    tensor_2d = out.squeeze()  # squeeze()会删除所有维度为1的轴，结果形状为(70,70)
    # 转换为NumPy数组（matplotlib只能处理NumPy）
    array_2d = tensor_2d.detach().numpy()  # 若张量在GPU上，需先转到CPU
    ...
    # 方案1：灰度图（推荐，适合图像类数据）
    plt.imshow(array_2d, cmap='gray')  # cmap='gray'指定灰度配色

    # 方案2：热力图（适合看数值分布，注释掉方案1可启用）
    # plt.imshow(array_2d, cmap='viridis')  # viridis是常用的热力图配色
    # plt.colorbar()  # 显示颜色刻度条，直观查看数值对应颜色

    # 美化设置
    plt.title('70x70 Tensor Visualization (1,1,70,70)')  # 标题
    plt.axis('off')  # 关闭坐标轴，更清晰
